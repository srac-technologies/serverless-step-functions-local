...
class ServerlessStepfunctionsLocal {
  ...
  async seedHandler() {
    ...
    const ensureUp = async (time, limit) => {
      ...
      try {
        ...
        return new Promise((resolve) => {
          setTimeout(() => {
            ensureUp(time + 1, limit).then(resolve)
          }, 100)
        })
      }
    }
    ...
    if (!await ensureUp(0, 10)) {
      throw new Error('the stepfunctions instance does not start within specified time (1,000ms)')
    }
    ...
    const recursivelyReplace = (obj) => {
      ...
      if ('Resource' in obj && ('Fn::GetAtt' in obj.Resource)) {
        return {
          ...obj, Resource: getFunctionName(obj.Resource['Fn::GetAtt'][0])
        }
      }

      if ('Resource' in obj && ('!GetAtt' in obj.Resource)) {
        return {
          ...obj, Resource: getFunctionName(obj.Resource['!GetAtt'][0])
        }
      }
      return obj
    }
